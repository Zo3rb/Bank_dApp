...
function App() {
  ...
  const contractABI = BankContract.abi;
  ...
  const depositMyMoneyHandler = async (event) => {
    try {
      event.preventDefault();
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        ...
        <div className="mt-7 mb-9">
          <form className="form-style">
            ...
            <button
              ...
              onClick={depositMyMoneyHandler}>Deposit Money In ETH</button>
          </form>
        </div>
